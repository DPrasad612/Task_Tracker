...
- `artifacts/task-tracker/src/` — React frontend
  ...
  - `types/index.ts` — shared TypeScript interfaces (includes `isSample` on Task)
...
- `lib/db/src/schema/` — Drizzle schema: users, preferences, tasks (with isSample), progress_logs
...
- **isSample flag**: Sample tasks seeded via "Load Sample Tasks" are marked `isSample=true` and can be cleared separately via `DELETE /api/tasks/sample`
...
- Sample tasks show "sample" badge; "Clear Samples" button removes them without touching real tasks
